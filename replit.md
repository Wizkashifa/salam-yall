...
- `user_accounts` table: Stores users (apple_id, google_id, email, display_name). Supports Apple Sign-In (iOS) and Google Sign-In (web). Google auth uses `POST /api/auth/google` with JWT verification via Google JWKS. Session tokens stored in `user_sessions` table; persisted via AsyncStorage on client (`auth_session_token` key). Env: `GOOGLE_CLIENT_ID` and `EXPO_PUBLIC_GOOGLE_CLIENT_ID`.
